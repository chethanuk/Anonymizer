...
# En and em dashes separate clauses and stay boundaries in both.
...
# braces, semicolons, operators, backticks, backslashes, "::", "->", a call "name(",
# snake_case or camelCase.
_CODE_MARK_RE = re.compile(r"[{};=<>`\\]|::|->|\w\(|[A-Za-z0-9]_[A-Za-z0-9]|[a-z][A-Z]")
...
def is_code_like(text: str) -> bool:
    ...
    """
    tokens = text.split()
    hits = sum(1 for token in tokens if _CODE_MARK_RE.search(token))
...
def _find_all_occurrences(text: str, needle: str, *, code_like: bool = False) -> list[tuple[int, int]]:
    if not needle:
        return []
    token_chars = _CODE_TOKEN_CHARS if code_like else _TOKEN_CHARS
    escaped = re.escape(needle)
    if needle[0].isalnum() or needle[0] == "_":
        escaped = rf"(?<![{token_chars}]){escaped}"
    if needle[-1].isalnum() or needle[-1] == "_":
        escaped = rf"{escaped}(?![{token_chars}])"
